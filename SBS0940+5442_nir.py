...
full_tbl = Table.read(os.getcwd()+'/test_data/optical+nir/SBS0940+5442_nir', format='ascii', delimiter=' ')
...
#    y_plus, temp, log_dens, c_Hb, a_H, a_He, tau_He, n_HI = theta
	...
	# Model flux
	model_flux = np.zeros(y.size) # emission lines we want to model
	...
	for w in range(len(emis_lines)):
		...
		if nearest_wave in hydrogen_lines and nearest_wave != 3890.166:
			...
			collisional_to_recomb_ratio = 0. #mfr.hydrogen_collision_to_recomb(xi, emis_lines[w], temp)
...
#				10**-(reddening_function * c_Hb)
		...
		# Infrared HeI10830 line
			...
			Pg_to_Hb_flux = emissivity_ratio * ( (EW_Hb + a_H)/(EW_Hb) ) / ( (EW_Pg + a_H_at_wave)/(EW_Pg) ) * 10**-(reddening_function * c_Hb)
...
np.save('{0:s}_{1:d}walkers_{2:d}steps'.format('SBS0940+5442', nwalkers, nmbr), sampler.chain)
