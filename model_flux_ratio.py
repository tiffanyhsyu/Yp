...
hydrogen_emis = Table.read(path+'/tables/hydrogen_emissivity_short', format='ascii', delimiter='\t')
...
# Vacuum wavelengths of Paschen-gamma and Balmer lines Ha, Hb, Hg, Hd, H8 for MCMC
hydrogen_lines = np.array([6564.612, 4862.721, 4341.684, 4102.891, 3890.166])
#hydrogen_lines = np.array([10941.11, 6564.612, 4862.721, 4341.684, 4102.891, 3890.166])
...
def hydrogen_emissivity(wave, temp, dens):
    ...
    if line == 'Hb':
        ...
        # Find indices in the hydrogen_emissivity Table corresponding to this transition, as well as H-beta, for the ratio
        line_idx = np.intersect1d(np.where(hydrogen_emis['Nu'] == upper_level)[0], np.where(hydrogen_emis['Nl'] == lower_level)[0])
        hb_idx = np.intersect1d(np.where(hydrogen_emis['Nu'] == 4)[0], np.where(hydrogen_emis['Nl'] == 2)[0])
        # Interpolate the emissivities
        hb_interp = interp.RectBivariateSpline(np.linspace(2.6, 4.7, num=22), np.arange(0, 9), hydrogen_emis[hb_idx]['emissivity'].reshape(22, 9), kx=1, ky=1)
        line_interp = interp.RectBivariateSpline(np.linspace(2.6, 4.7, num=22), np.arange(0, 9), hydrogen_emis[line_idx]['emissivity'].reshape(22, 9), kx=1, ky=1)
        # Calculate emissivities at desired temperature, density
        hb_emis = hb_interp(log_T, log_ne)[0][0]
        line_emis = line_interp(log_T, log_ne)[0][0]

        Xt = line_emis / hb_emis

    return Xt

def hydrogen_emissivity_old(wave, temp, dens):
    '''
    Calculate the emissivity of a hydrogen line
    relative to H(beta).
    ...
    if line == 'Hb':
        Xt = 1.
    # From AOS 2010 above Eq. 4.1; fitted to data from Hummer & Storey:
    elif line == 'H8':
        Xt = 0.104 * (T4**0.046)
    # From Osterbrock 1989 Table 4.4; estimated Pg/Hd * Hd/Hb for T=10^4K, n_e=10^2cm^-3
    elif line == 'Pg':
        Xt = 0.090391
    else:
        ...
        for i in range(cij.shape[0]):
            for j in range(cij.shape[1]):
                ...
                # This formulation might be best fit for only 12000≤T≤20000K and 1≤ne≤300 cm−3, although
                # this is only stated for helium fit (AOS 2010 between Eq. 3.1 and 3.2)
...
HeI_emis_10833 = interp.RectBivariateSpline(np.logspace(1, 14, num=14), np.linspace(5e3, 25e3, num=21), helium_emis['10830A'].reshape((14, 21)), kx=1, ky=2)
...
def stellar_absorption(wave, a_default, ion=None):
    ...
    if ion in ['hydrogen', 'Hydrogen', 'H']:
        ...
        H_idx = np.where(np.abs(hydrogen_lines - wave) < 3)[0]

        if H_idx == 0: # Pa-g
            a_at_wave = 0.4 * a_default # From Section 4 of AOS 2015
        elif H_idx == 1: # Ha
    ...
    # Underlying HeI stellar absorption
        ...
        if He_idx == 0: # HeI10833
            a_at_wave = 0.800 * a_default # From end of Section 2 in AOS2015; Eq. 2 in ITG 2014
...
def optical_depth_function(wave, temp, dens, tau):
    ...
    elif idx == 11: #11th index corresponds to the row in Table helium_optial_depth that HeI10833 is in (I added it in the table that we read in; copied from Eq. 2.2 of AOS2015)
...
def reddening_coefficient(wave):
    ...
    if x >= 0.3 and x < 1.1: # Corresponds to 9090-33300 Angstroms
        a = 0.574 * (x ** 1.61)
        b = -0.527 * (x ** 1.61)

    elif x >= 1.1 and x <= 3.3: # Corresponds to 3030-9090 Angstroms
        y = x - 1.82
...
# All assume EW(HeI3889) = 0.5 * EW(3889)
#            EW(H8) = 0.5 * EW(3889)
################################
...
#def generate_emission_line_ratio(filename, waves, EWs, EW_Hb, y_plus, temp, dens, c_Hb, a_H, a_He, tau_He, n_HI):
    ...
    collisional_to_recomb_Hbeta = hydrogen_collision_to_recomb(xi, hydrogen_lines[1], temp)
    f_lambda_at_Hbeta = f_lambda_avg_interp(hydrogen_lines[1])
...
#def generate_emission_line_ratio(filename, waves, EWs, EW_Hb, y_plus, temp, dens, c_Hb, a_H, a_He, tau_He, n_HI):
    ...
    collisional_to_recomb_Hbeta = hydrogen_collision_to_recomb(xi, hydrogen_lines[1], temp)
    AHbeta_AV = reddening_coefficient(hydrogen_lines[1])
