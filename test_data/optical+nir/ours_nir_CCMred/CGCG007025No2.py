...
full_tbl = Table.read(os.getcwd()+'/test_data/CGCG007025No2_nir', format='ascii', delimiter=' ')
...
#    y_plus, temp, log_dens, c_Hb, a_H, a_He, tau_He, n_HI = theta
	...
	# Model flux
	model_flux = np.zeros(y.size) # 11 emission line fluxes we want to model
	...
	AHbeta_Av = mfr.reddening_coefficient(hydrogen_lines[2])
...
#				10**-(reddening_function * c_Hb)
		...
		# Infrared HeI10830 line
			...
			reddening_function = ( mfr.reddening_coefficient(10941.082) / AHbeta_Av ) - 1. # hard-coded Pg wavelength; could also be hydrogen_lines[0]
...
#    y_plus, temp, log_dens, c_Hb, a_H, a_He, tau_He, n_HI = theta
	...
	if min_y_plus <= y_plus <= max_y_plus and \
		...
		return ( -(temp - 16500)**2 / (0.2*16500)**2 )
...
np.save('{0:s}_{1:d}walkers_{2:d}steps'.format('CGCG007025No2', nwalkers, nmbr), sampler.chain)
