...
full_tbl = Table.read(os.getcwd()+'/test_data/SBS1152+579_nir', format='ascii', delimiter=' ')
...
#    y_plus, temp, log_dens, c_Hb, a_H, a_He, tau_He, n_HI = theta
	...
	if min_y_plus <= y_plus <= max_y_plus and \
		...
		return ( -(temp - 15400)**2 / (0.2*15400)**2 )
...
np.save('{0:s}_{1:d}walkers_{2:d}steps'.format('SBS1152+579', nwalkers, nmbr), sampler.chain)
