...
flux_ratios = Table.read(os.getcwd()+'/test_data/Tol65', format='ascii', delimiter=' ')
...
def get_model(theta):
	...
	for w in range(len(emis_lines)):
		...
		if nearest_wave in balmer_lines and nearest_wave != 3890.166:
			...
			#collisional_to_recomb_ratio = mfr.hydrogen_collision_to_recomb(xi, emis_lines[w], temp) #S2018 includes 
...
#				10**-(reddening_function * c_Hb)
			...
			flux = ( emissivity_ratio * 10**-(reddening_function * c_Hb) * ( (EW_Hb + a_H)/(EW_Hb) ) ) - ( (a_H_at_wave / EW_Hb) * (h[w]) )
...
def lnprior(theta):
	...
	if min_y_plus <= y_plus <= max_y_plus and \
		...
		return ( -(temp - 17200)**2 / (0.2*17200)**2 ) # Test
...
np.save('{0:s}_{1:d}walkers_{2:d}steps'.format('Tol65', nwalkers, nmbr), sampler.chain)
